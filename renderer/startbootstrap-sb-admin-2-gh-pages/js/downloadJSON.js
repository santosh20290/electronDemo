...
let action;
// Dom Nodes
function getCaptcha(type, btnObj, userAction) {
    btnElement = btnObj;
    returnType = type;
    action = userAction;
...
function setCaptcha(event) {
    ...
    console.log(action);
    if(returnType === "gstr1" && action === "downloadGstr1JSON"){
        ipcRenderer.send('download-json', userName, gstnPassword, captchaValue, returnPeriod.value, returnType);
    }if(returnType === "gstr1" && action === "uploadGstr1JSON"){
        ipcRenderer.send('upload-json', userName, gstnPassword, captchaValue, returnPeriod.value, returnType);
...
ipcRenderer.on('get-captcha-success', (e, response) => {
    ...
    }else if(returnType === "gstr1" && action === "uploadGstr1JSON"){
        btnName = "Upload JSON";
    }else if(returnType === "gstr2a"){
...
ipcRenderer.on('upload-json-success', (e, res) => {
...
ipcRenderer.on('upload-json-error', (e, res) => {
    toggleModalButtons(btnElement);
    console.log(res);
    notifyMessage("danger", "There must be some problem with GSTN. Please try again later")
});
